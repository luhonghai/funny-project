...
<img src="{$membersprofilepicurl}/{if $topusers[i].profilepicture eq ""}noprofilepicture.jpg{else}{$topusers[i].profilepicture}{/if}?{$smarty.now}" style="width:60px;height:60px;">
                    ...
                    <div class="meta">
                      <a style="font-size:16px; font-weight: bold; padding-bottom: 5px; margin-bottom: 6px; border-bottom: 1px solid rgb(242, 242, 242); display: block;"><strong>{$topusers[i].fullname}</strong></a>
                    	<span style="font-size: 12px; color: rgb(68, 68, 68);">Nick : <a href="{$baseurl}/user/{$topusers[i].username}"><strong>{$topusers[i].username}</strong></a>&nbsp;&nbsp;<span style="color: rgb(238, 238, 238);">|</span>&nbsp;&nbsp;Bài đăng: <strong>{$topusers[i].TOTAL}</strong>&nbsp;&nbsp;<span style="color: rgb(238, 238, 238);">|</span>&nbsp;&nbsp;Tổng số điểm: <strong>{$topusers[i].LIKES}</strong><span style="color: rgb(238, 238, 238);">|</span>&nbsp;&nbsp;Tổng lượt xem: <strong>{$topusers[i].VIEWS}</strong></strong><span style="color: rgb(238, 238, 238);">
