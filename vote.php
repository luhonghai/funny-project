...
if($SID > 0)
{
	...
	$query1 = "SELECT count(*) as total from posts A, members B where A.active='1' AND A.USERID=B.USERID AND A.phase='0' $gridsql order by A.time_added desc limit $config[maximum_results]";
	$query2 = "SELECT A.*, B.username from posts A, members B where A.active='1' AND A.USERID=B.USERID AND A.phase='0' $gridsql order by A.time_added desc limit $pagingstart, $config[items_per_page]";
	...
	else
	{
		if($grid == "1")
		{
			$templateselect = "vote_grid.tpl";
		}
		else
		{
			$templateselect = "vote_std.tpl";
		}
		if ($totalvideos > 0)
		{
			if($executequery1->fields['total']<=$config['maximum_results'])
